...
object PluginCompatibilityUtils {
    ...
    private fun checkVersionAndOs(targetVersion: String, supportedOs: List<org.wip.plugintoolkit.api.OS>): Pair<Boolean, String?> {
        ...
        if (supportedOs.isNotEmpty()) {
            val currentOs = when {
                FileUtils.isWindows -> org.wip.plugintoolkit.api.OS.WINDOWS
                FileUtils.isLinux -> org.wip.plugintoolkit.api.OS.LINUX
                FileUtils.isMac -> org.wip.plugintoolkit.api.OS.MACOS
                else -> null
            }
