...
object MigrationEngine {
    ...
    fun reconcileCapabilityNodePorts(
        ...
        // Re-evaluate broken state: a node should only be broken if a required parameter without default or
        // autogenerated pattern is newly introduced and missing from the existing inputs.
        var isNodeBroken = false
        for ((key, meta) in inputParams) {
            val wasMissing = existingInputs.none { it.id == key }
            if (wasMissing && meta.required && meta.defaultValue == null && meta.autogeneratedPattern == null) {
                isNodeBroken = true
            }
    ...
    /**
     * Refreshes a capability node against a current manifest, reconciling ports and healing
     * the node if the capability is found and valid.
     */
    fun refreshCapabilityNode(
        node: Node.CapabilityNode,
        manifest: PluginManifest
    ): Node.CapabilityNode {
        val actualCapability = manifest.capabilities.find { it.name == node.capability.name }
            ?: return node
        return reconcileCapabilityNodePorts(node, actualCapability, manifest.plugin)
    }
     ...
     * @param currentManifests A map of plugin ID to its current Manifest.
     * @param getMigrations A function to fetch the migrations for a given plugin ID.
     * @param targetPluginId Optional ID of a specific plugin that was updated. If provided,
     *                       nodes from other plugins are left untouched and will NOT be marked broken.
     */
    suspend fun migrateFlow(
        ...
        if (currentManifests.isEmpty()) {
            // Guard: If no manifests are provided (e.g. early startup before registry is ready),
            // do not destructively mark nodes as broken.
            return MigrationResult(flow, emptyList(), emptyList())
        }
        ...
        for (node in flow.nodes) {
            ...
            // If no plugin provides this capability:
            // If this is a targeted update for another plugin, DO NOT break this node.
            if (currentManifest == null) {
