...
class PluginScanner(
    ...
    suspend fun rescanManagedFolders() {
        ...
        folders.forEach { folderPath ->
            ...
            val existingContent = FileUtils.readFile(trackingFile)
            ...
            FileUtils.listDirectories(normalizedFolder).forEach { dirPath ->
                ...
                jarFiles.forEach { jarPath ->
                    ...
                    val manifestContent = FileUtils.readFileFromZip(normalizedJarPath, "manifest.json")
                        ?: FileUtils.readFileFromZip(normalizedJarPath, "META-INF/manifest.json")
            ...
            if (folderChanged || !FileUtils.exists(trackingFile)) {
                Logger.i { "Updating tracking file: $trackingFile with ${folderPlugins.size} plugins" }
                FileUtils.writeFile(trackingFile, json.encodeToString(folderPlugins))
                globalChanged = true
            }
