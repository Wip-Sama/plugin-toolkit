...
Changes:
  ...
  - Isolated all third-party plugin lifecycle operations
...
Changes:
	...
	- System nodes correctly have path/file path/folder 
	- SemanticTypes with only the base will accept any SemanticType with that base (e.g. path will accept path/file)
	- The search bar now filters system nodes too
	- Now to move around in the flow editor use right-click, to select multiple nodes use left-click 
	- Auto Scroll is now toggleable in execution
Added:
	...
	- option to add default to the system input node
	- Padding while holding a connection in the flow editor
	- Flow version and description editor fields
	- Export flow to string (basically gzip+base64) to copy paste flows more easily in chat
	- Pressing on a flow navigate to the flow runner to that specific flow 
  - Pressing on a plugin navigates to the capability runner for that plugin (when possible)
  - Middle click (hold) to pan inside the flow editor
  - Individual progress bar in flows
  - Resizeable flow logs
  - Customizable flow logs history size
  - Log export to jobs
  - OutputData is now selectable 
Fixed:
	- Output path field would appear required even though it would be autogenerated/not needed
