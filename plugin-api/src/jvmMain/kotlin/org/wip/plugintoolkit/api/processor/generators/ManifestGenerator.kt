...
object ManifestGenerator {
    ...
    fun generateManifestObject(
        ...
        functions.forEachIndexed { index, func ->
            ...
            paramsList.forEachIndexed { pIndex, param ->
                ...
                if (!isInfrastructure) {
                    ...
                    capabilitiesCode.add(
                        "%S to %T(defaultValue = %L, description = %S, type = %L, constraints = %L, required = %L, secret = %L, semanticTypes = %L, role = %L, autogeneratedPattern = %L, isDestructive = %L)",
                        paramNameStr,
        ...
        settingsProperties.forEachIndexed { index, prop ->
            ...
            val requiredByCapabilities = functions.mapNotNull { function ->
                val capabilityAnnotation = function.annotations.find {
                    it.hasQualifiedName(CAPABILITY_ANNOTATION)
                } ?: return@mapNotNull null
                val requiredSettings =
                    (capabilityAnnotation.arguments.find { it.name?.asString() == "requiresSettings" }?.value as? List<*>)
                        ?.filterIsInstance<String>()
                        ?: emptyList()
                if (propName !in requiredSettings) return@mapNotNull null
                capabilityAnnotation.arguments.find { it.name?.asString() == "name" }?.value as? String
            }
            ...
            val hasConstraints =
                !minValue.isNaN() || !maxValue.isNaN() || minLength != -1 || maxLength != -1 ||
            ...
            val constraintsCode = if (hasConstraints) {
                CodeBlock.of(
                    ...
                    if (minLength != -1) minLength else "null",
                    if (maxLength != -1) maxLength else "null",
                    if (regex.isNotEmpty()) CodeBlock.of("%S", regex) else CodeBlock.of("null"),
                    if (multiSelect) "true" else "null",
                    if (minChoices != -1) minChoices else "null",
                    if (maxChoices != -1) maxChoices else "null"
                )
            } else CodeBlock.of("null")
            val semanticTypesCode = generateSemanticTypesCode(
                semanticTypeValues.flatMap { org.wip.plugintoolkit.api.parseSemanticTypes(it) }
            )
            val autogeneratedPatternCode =
                if (pathTemplate.isBlank()) CodeBlock.of("null") else CodeBlock.of("%S", pathTemplate)
            ...
            settingsCode.add(
                "%S to %T(defaultValue = %L, description = %S, type = %M<%T>(), constraints = %L, required = %L, secret = %L, semanticTypes = %L, autogeneratedPattern = %L, requiredByCapabilities = %L)",
                propName,
                CN_SETTING_METADATA,
                defaultValueCode,
                desc,
