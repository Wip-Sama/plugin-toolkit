...
object ManifestJsonGenerator {
    fun generate(
        ...
        val manifestCapabilities = functions.map { func ->
            ...
            val params = GeneratorUtils.getCapabilityParameters(func).associate { param ->
                val defaultJson = if (param.defaultValue.isNotEmpty()) {
                    try {
                        Json.parseToJsonElement(param.defaultValue)
                    } catch (e: Exception) {
                        kotlinx.serialization.json.JsonPrimitive(param.defaultValue)
                    }
                } else null
                ...
                param.name to ParameterMetadata(
                    ...
                    role = param.role,
                    autogeneratedPattern = param.autogeneratedPattern,
                    isDestructive = param.isDestructive,
