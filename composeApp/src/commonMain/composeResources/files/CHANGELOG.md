...
Added:
	- When hoverina node connection the related noted to that connection will be highlighted and the others dimmed (TODO)
	- Copy pasting nodes and group of nodes (TODO)
	- A toast when a plugin fails to install to let the user know the reason (TODO)
Fixed:
	- Output path field would appear required even thought it would be autogenerated/not needed (TODO)
	- DropdownMenu in nodes would appear distant / hover the dropdown box (the distance was not scaling properly) (TODO)
----------------------------------------------------------------------------------------------------
Version: 1.7.2
Date: 13-07-2026
Fixed:
