...
class PluginViewModel(
    ...
    private val manuallyModifiedParameters = mutableSetOf<String>()
    private val jobCounterMutex = Mutex()
    private var jobCounter = 0

    fun updateParameter(name: String, value: String) {
        parameterValues[name] = value
        manuallyModifiedParameters.add(name)
    ...
    private fun recomputeAutogeneratedParameters() {
        ...
        for ((outName, meta) in params) {
            ...
            if (meta.role == org.wip.plugintoolkit.api.ParameterRole.OUTPUT_LOCATION && !pattern.isNullOrBlank()) {
                if (manuallyModifiedParameters.contains(outName)) continue
                
                var resolvedPath = pattern!!
                for ((inName, inValue) in parameterValues) {
                    resolvedPath = resolvedPath.replace("{$inName}", inValue)
                }
