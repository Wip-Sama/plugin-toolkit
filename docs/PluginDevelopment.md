...
Use the runtime API and KSP processor at the same toolkit version:
...
> **Required when upgrading to 2.0:** `plugin-api` is now runtime-only. A build that still declares
> `ksp("org.wip.plugintoolkit:plugin-api:…")` may succeed without running any processor and will produce an
> unusable plugin with no generated manifest or entry point. Change the KSP dependency to `plugin-processor`.
