...
1. **Internal Operations (PluginFileSystem & ExecutionFileSystem)**: When your plugin needs to save internal data, temporary processing files, or downloaded models, use `PluginFileSystem` (for persistent data) or `ExecutionFileSystem` (for temporary, per-job data). 
   
   Both interfaces inherit from a common **`ScopedFileSystem`** base interface which defines standard, relative file operations:
   - `readFile(relativePath)` / `readTextFile(relativePath)`
   - `writeFile(relativePath, data)` / `writeTextFile(relativePath, text)`
   - `exists(relativePath)`
   - `listFiles(relativePath)`
   - `deleteFile(relativePath)`
   - `getBasePath()` (gets the absolute base path of the managed folder)

   `PluginFileSystem` additionally supports:
   - `extractResource(resourcePath, targetRelativePath)` (for extracting native tools/scripts bundled in the JAR to the plugin's persistent data directory)
...
To declare that a capability uses the `HostFileSystem`, annotate the path parameters with `@CapabilityInput` (for reading) and `@CapabilityOutput` (for writing). The toolkit automatically infers the required permissions:

```kotlin
@Capability(name = "Process External File")
fun processFile(
    @CapabilityInput(description = "Input File") inFile: String,
    @CapabilityOutput(description = "Output File", autogeneratedPattern = "{inFile}_processed") outFile: String,
...
> [!WARNING]
> Unmapped Output Auto-Routing: If a flow contains a capability with a parameter marked as `@CapabilityOutput`, but the user leaves the parameter empty (unmapped) in the flow editor, the host application will **automatically route that output into the Execution Sandbox** by generating a temporary `.tmp` path. This ensures intermediate files between flow nodes are cleanly sandboxed and deleted after execution.
