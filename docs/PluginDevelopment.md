...
Parameters can be annotated with different annotations depending on their role in file access operations:

1. **`@CapabilityParam`**: Standard parameter input.
2. **`@CapabilityInput`**: Used to mark a parameter that represents an input file or folder path. The host application will present this as an `Input Location` field.
3. **`@CapabilityOutput`**: Used to mark a parameter that represents an output file or folder path. The host application will present this as an `Output Location` field. It supports two special properties:
...
The host application will automatically infer the required file access permissions based on the presence of `@CapabilityInput` and `@CapabilityOutput` annotations in your capability.
...
fun processFile(
    @CapabilityInput @CapabilityParam(description = "Input File") inFile: String,
    @CapabilityOutput(autogeneratedPattern = "{inFile}_processed") @CapabilityParam(description = "Output File") outFile: String,
